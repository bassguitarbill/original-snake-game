...
void spawn_snake(Game* game, int start_x, int start_y);
void move_snake(Game* game);
void change_direction(Game* game, SDL_KeyCode new_direction);
void add_control_input_to_buffer(Game* game, SDL_KeyCode new_direction);
...
void unpause_game(Game* game);
void to_title_screen(Game* game);

void initialize_game_object(Game* game);
...
int main() {
  ...
  initialize_game_object(game);
  initialize(game);

  spawn_snake(game, SNAKE_START_X, SNAKE_START_Y);
  spawn_food(game);
...
void spawn_snake(Game* game, int start_x, int start_y) {
  ...
  game->snake = calloc(SNAKE_SIZE, sizeof(SDL_Rect));
...
void pop_movement(Game* game) {
  if (game->input_buffer->count == 0) return;
  change_direction(game, pop_from_input_buffer(game->input_buffer));
}
        ...
        if (e.type == SDL_KEYDOWN) {
          if (e.key.keysym.sym == SDLK_UP
           ...
           || e.key.keysym.sym == SDLK_RIGHT) {
            add_control_input_to_buffer(game, e.key.keysym.sym);
